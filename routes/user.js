...
const Visitations = require('../models/Visitations');
...
router.delete('/', async (req, res) => {
    ...
    await Homes.deleteMany({ owner_id: user_id });
    // delete all land where owner_id belongs to user
    await Land.deleteMany({ owner: user_id });
    //delete all meetings where invitor_id or invited_id belongs to user
    await Meetings.deleteMany({
        $or: [{ invitor_id: user_id }, { invited_id: user_id }],
    });
    // Delete all the Messages where sender_id or receiver_id belongs to user.
    await Messages.deleteMany({
        $or: [{ sender_id: user_id }, { receiver_id: user_id }],
    });
    // Delete all the Visitations where organizator_id belongs to user.
    await Visitations.deleteMany({ organizator_id: user_id });
